...
(function () {
    ...
    /**
     * 校验是否重复添加--> methodName checkIsRepetition
     * param = [0,1,3]
     * 0 所有数据
     * 1 待验证的 arg 参数
     * 2 是否忽略字符串数字类型进行比较
     */
    jQuery.validator.addMethod("checkIsRepetition",
        function (value, element, param) {
            if (param && param.length === 3) {
                var objects = param[0];
                var arg = param[1];
                var ignoreNumberStringType = param[2];
                var elementInJSONObject = checkArgs.isElementInJSONObject(objects(), arg(), ignoreNumberStringType);
                return !elementInJSONObject;
            }
            return false;
        }
        , "添加的内容已定义"),

        // IP地址验证--> methodName ip
        ...
        // port验证--> methodName port
        jQuery.validator.addMethod("port",
            function (value, element) {
        ...
        //手机号码--> methodName mobile
        jQuery.validator.addMethod("mobile",
            function (value, element) {
                //var patrn=/^[1]([3][0-9]{1}|59|58|88|89)[0-9]{8}$/;
                //不对前三位进行验证
                var patrn = /^[1]([0-9]{2})[0-9]{8}$/;
                return this.optional(element) || patrn.test(value);
            }
            , "请填写正确的手机号码"),

        //正则表示式--> methodName regularExpression
        //默认取绑定的校验元素的  regularExpression 属性
        jQuery.validator.addMethod("regularExpression",
            function (value, element) {
                var patrn = $(element).attr("regularExpression");
                return this.optional(element) || new RegExp(patrn).test(value);
            }
            , "请填写有效的时间"),

        //文本框内容使用「,」逗号分割后内容数量与 select 框选中内容数量是否一致--> methodName splitLengthIsEqSelectVal
        //默认取绑定的校验元素的  param 属性 作为被校验Id
        jQuery.validator.addMethod("splitLengthIsEqSelectVal",
            function (value, element, param) {
        		var specialChar = /[^0-9\.,，\s]+/;
        		// 非空验证及特殊符限制
        		if(value.trim() == "" || specialChar.test(value)){
        			return false;
        		}
        		return value.split(/\s*[,，]\s*/).length === $(param).length;
            }
            , "填写内容数量与所选内容数量不一致"),
    ...
    for (b = 0; b < 17; b++) {
        ...
        if (!isNaN(d))d *= e; else for (c = 0; c < h.length; c++)if (d.toUpperCase() === h[c]) {
    ...
    var d = typeof c == "string" ? c.replace(/,/g, "|") : "image/*", e = this.optional(b), f, g;
    if (e)return e;
    if ($(b).attr("type") === "file") {
        d = d.replace("*", ".*");
        if (b.files && b.files.length)for (f = 0; f < b.files.length; f++) {
            g = b.files[f];
