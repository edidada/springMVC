(function($) {
    ...
    MultiSuggest.prototype = $.extend(MultiSuggest, _superproto, {
        ...
        select: function () {
            ...
            var _moreInfo = active.attr('data-moreInfo');
            this.setValue(display, val, _moreInfo);
        },
        setValue : function(display, value, _moreInfo) {
            if (display && display !== "" &&
                value && value !== "") {
                this.$element.val(this.updater(display)).change();
                this.$hiddenInput.val(value);
                this.$element.addClass("msuggest-selected");
                this.$element.trigger("ezmultisuggest",[value,display,_moreInfo]);
            }
            return this.hide();
        },
        ...
        _normalize : function(dataSource) {
            ...
            dataSource.results = $.map(dataSource.results, function(item) {
                    ...
                    idAttr = dataSource.valueAttribute(item);
					  var _moreinfo = null;
					  if(dataSource.ezMoreInfo != null){
						  _moreinfo = dataSource.ezMoreInfo(item);
					  }
        ...
        _renderMenu : function(ul, dataSource) {
            ...
            items = $(items).map(function(i, item) {
                i = $(self.options.item).attr('data-value', item.value)
                                        .attr('data-display', item.displayValue)
										    .attr('data-moreInfo',item.moreInfo)
                                        .addClass("menuitem");
            ...
            if (!items.length) {
                ...
                //var item_display = items.slice(0,self.options.maxItemDisplay); //by E_Bo
                ul.append(items);                  
            }
		 ...
		 show: function () {
		  ...
		  if(this.options.dropup){
			  this.$menu
				...
				.css({
				  top: pos.top - this.$menu.outerHeight(true) - 2,
				  left: pos.left
				}).show();
        ...
        //添加销毁方法
        destroy : function () {
            this.$element.data('multisuggest',null);
            this.$element
                .off('focus')
                .off('blur')
                .off('keypress')
