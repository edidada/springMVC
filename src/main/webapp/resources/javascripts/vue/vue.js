...
var text = Object.freeze({
  ...
  function applyTransition(el, direction, op, vm, cb) {
    ...
    if (!transition ||
    // skip if there are no nodejs hooks and CSS transition is
    // not supported
...
var template = Object.freeze({
    ...
    bind: function bind() {
      ...
      if (tokens) {
        ...
        if (disallowedInterpAttrRE.test(attr) || attr === 'name' && (tag === 'PARTIAL' || tag === 'SLOT')) {
          'development' !== 'production' && warn(attr + '="' + descriptor.raw + '": ' + 'attribute interpolation is not allowed in Vue.nodejs ' + 'directives and special attributes.');
  ...
  function flush() {
    ...
    queue$1 = [];
    queued = false;
    // dummy return, so nodejs linters don't complain about
    // unused variable f
    return f;
  }
   ...
   * 5. call enter hook (with possible explicit nodejs callback)
   * 6. reflow
   * 7. based on transition type:
   *    - transition:
   *        remove class now, wait for transitionend,
   *        then done if there's no explicit nodejs callback.
   *    - animation:
   *        wait for animationend, remove class,
   *        then done if there's no explicit nodejs callback.
   *    - no css transition:
   *        done now if there's no explicit nodejs callback.
   * 8. wait for either done or nodejs callback, then call
   *    afterEnter hook.
   *
   ...
   * 4. call leave hook (with possible explicit nodejs callback)
   * 5. reflow if no explicit nodejs callback is provided
   * 6. based on transition type:
   *    - transition or animation:
   *        wait for end event, remove class, then done if
   *        there's no explicit nodejs callback.
   *    - no css transition:
   *        done if there's no explicit nodejs callback.
   * 7. wait for either done or nodejs callback, then call
   *    afterLeave hook.
   *
  ...
  p$1.leave = function (op, cb) {
    ...
    // 2. there's no explicit nodejs callback
  ...
  p$1.getCssTransitionType = function (className) {
    ...
    // CSS transitions.
    document.hidden ||
    // explicit nodejs-only transition
