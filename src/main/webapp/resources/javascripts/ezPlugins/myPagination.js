		...
		if (param && param instanceof Object) {
			...
			var defaults = new Object({
				...
				info : {
					...
					msg_on : true,
					link : '#',
					msg : '<span>&nbsp;&nbsp;{currText}/{sumPage}页&nbsp;&nbsp;共{totalCount}条</span>',
					text : {
						width : '35px'
					}
				}
			...
			function getMsg() {
				...
				if (options.info && options.info.msg) {
					...
					str = str.replace("{totalCount}", totalCount);
					return str
				} else {
...
(function($) {
	$.fn.fPagination = function(opts) {
		$(this).myPagination(opts);
		$allAItem = $(this).children("a");
		$allAItem.unbind("click");
		$allAItem.bind("click",function(){
			opts.callback($(this).attr("title"));
	    });
	    ...
	    $text.keypress(function(event){
	        ...
	        if(keycode==13){
	            var page = $(this).val();
	            if(isNaN(page) || page < 1 || page > opts.pageCount){
	                alert("请输入有效页数!");
	                return false;
	            }
	            opts.callback(page);
